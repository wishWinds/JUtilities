//
//  UIImage+Utilities.m
//  Pods-Utilities_Example
//
//  Created by shupeng on 2018/7/26.
//
...
static NSTimeInterval _J_CGImageSourceGetGIFFrameDelayAtIndex(CGImageSourceRef source, size_t index) {
...
+ (UIImage *)imageWithSmallGIFData:(NSData *)data scale:(CGFloat)scale {
    ...
    for (size_t i = 0; i < count; i++) {
        NSTimeInterval delay = _J_CGImageSourceGetGIFFrameDelayAtIndex(source, i);
        totalTime += delay;
...
+ (UIImage *)imageWithPDF:(id)dataOrPath {
    return [self _J_imageWithPDF:dataOrPath resize:NO size:CGSizeZero];
}

+ (UIImage *)imageWithPDF:(id)dataOrPath size:(CGSize)size {
    return [self _J_imageWithPDF:dataOrPath resize:YES size:size];
}
...
+ (UIImage *)_J_imageWithPDF:(id)dataOrPath resize:(BOOL)resize size:(CGSize)size {
...
- (void)drawInRect:(CGRect)rect withContentMode:(UIViewContentMode)contentMode clipsToBounds:(BOOL)clips {
    CGRect drawRect = JCGRectFitWithContentMode(rect, self.size, contentMode);
...
- (UIImage *)_J_flipHorizontal:(BOOL)horizontal vertical:(BOOL)vertical {
...
- (UIImage *)imageByRotate180 {
    return [self _J_flipHorizontal:YES vertical:YES];
}

- (UIImage *)imageByFlipVertical {
    return [self _J_flipHorizontal:NO vertical:YES];
}

- (UIImage *)imageByFlipHorizontal {
    return [self _J_flipHorizontal:YES vertical:NO];
}
...
- (UIImage *)imageByBlurRadius:(CGFloat)blurRadius
    ...
    if (self.size.width < 1 || self.size.height < 1) {
        NSLog(@"UIImage+JKit error: invalid size: (%.2f x %.2f). Both dimensions must be >= 1: %@", self.size.width, self.size.height, self);
        return nil;
    }
    if (!self.CGImage) {
        NSLog(@"UIImage+JKit error: inputImage must be backed by a CGImage: %@", self);
        return nil;
    }
    if (maskImage && !maskImage.CGImage) {
        NSLog(@"UIImage+JKit error: effectMaskImage must be backed by a CGImage: %@", maskImage);
        return nil;
    }
    ...
    if (!hasBlur && !hasSaturation) {
        return [self _J_mergeImageRef:imageRef tintColor:tintColor tintBlendMode:tintBlendMode maskImage:maskImage opaque:opaque];
    }
    ...
    if (hasNewFunc) {
        ...
        if (err != kvImageNoError) {
            NSLog(@"UIImage+JKit error: vImageBuffer_InitWithCGImage returned error code %zi for inputImage: %@", err, self);
            return nil;
        }
        err = vImageBuffer_Init(&scratch, effect.height, effect.width, format.bitsPerPixel, kvImageNoFlags);
        if (err != kvImageNoError) {
            NSLog(@"UIImage+JKit error: vImageBuffer_Init returned error code %zi for inputImage: %@", err, self);
            return nil;
        }
    } else {
    ...
    if (hasNewFunc) {
        ...
        effectCGImage = vImageCreateCGImageFromBuffer(input, &format, &_J_cleanupBuffer, NULL, kvImageNoAllocate, NULL);
        if (effectCGImage == NULL) {
            effectCGImage = vImageCreateCGImageFromBuffer(input, &format, NULL, NULL, kvImageNoFlags, NULL);
            free(input->data);
        }
        free(output->data);
        outputImage = [self _J_mergeImageRef:effectCGImage tintColor:tintColor tintBlendMode:tintBlendMode maskImage:maskImage opaque:opaque];
...
static void _J_cleanupBuffer(void *userData, void *buf_data) {
    free(buf_data);
}

// Helper function to add tint and mask.
- (UIImage *)_J_mergeImageRef:(CGImageRef)effectCGImage
                    tintColor:(UIColor *)tintColor
